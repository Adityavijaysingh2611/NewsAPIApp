...
const Default = () => {
  ...
  const url = `https://gnews.io/api/v4/search?q=${topic}&apikey=c643be2ea728d1e62052c04421fdbff6`;
  ...
  return (
    <main>
      ...
      {news.map((newsItem) => (
        ...
        <div className="article-con" key={newsItem.title}>
          <img src={newsItem.image} alt="" style={{width:"100%", borderRadius:"10px"}} />
          <h3>{newsItem.title}</h3>
          <p>{newsItem.content}</p>
          <button><a href={newsItem.url} target="_blank" without rel="noreferrer">Read more here </a></button>
          
          <p className="source">Source <span>{newsItem.source.name}</span></p>
        </div>
